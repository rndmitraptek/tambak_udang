...
app.controller("myCtrl", function($scope,$http) {
    ...
    var uuid_simulasi = "{{ $uuid_simulasi }}";
    ...
    $(document).ready(function() {
        ...
        $.getJSON('/transaksi-biaya-simulasi/biaya-list', function(data) {
        ...
        .on("change", "#siklus-perpetak, #petak-perpetak", function() {
            ...
            tablePetak.ajax.url("/transaksi-biaya-simulasi/petak-list?siklus_id[]=" + [siklus_id] + "&petak_id=" + petak_id).load();
        });
        ...
        function reloadPetakTable() {
            ...
            tablePetak.ajax.url("/transaksi-biaya-simulasi/petak-list?siklus_id[]=" + selectedSiklus.join("&siklus_id[]=")).load();
        }
        ...
        tablePetak = $('#viewtabelpetak').DataTable({
            ...
            serverSide: false,
            ajax: {
                url: "/transaksi-biaya-simulasi/petak-list",
                data: function(d) {
        ...
        $(document).on('click', '#btn-simpan', function() {
            ...
            var url = payload.uuid ? '/transaksi-biaya-simulasi/update/' + payload.uuid : '/transaksi-biaya-simulasi/store';
            $.ajax({
                url: url,
                type: "POST",
    ...
    $scope.editTransaksi = function(uuid) {
        $.get('/transaksi-biaya-simulasi/' + uuid, function(res) {
            $('#uuid').val(res.uuid);
            ...
            setTimeout(function () {
                ...
                if (kelompok === 'Perpetak') {
                    ...
                    setTimeout(function () {
                        ...
                        if (res.siklus[0] && res.siklus[0].petak.length > 0) {
                            ...
                            if (siklusId) {
                                ...
                                tablePetak.ajax
                                    .url(`/transaksi-biaya-simulasi/petak-list?siklus_id[]=${siklusId}&petak_id=${petakId}`)
                                    .load(function() {
                    ...
                    if (siklusIds.length > 0) {
                        tablePetak.ajax
                            .url("/transaksi-biaya-simulasi/petak-list?siklus_id[]=" + siklusIds.join("&siklus_id[]="))
                            .load(function() {
                                $scope.$applyAsync(function() {
                                    $scope.nominal = parseFloat(res.nominal);
                                });
                        ...
                        tablePetak.ajax.url("/transaksi-biaya-simulasi/petak-list").load(function() {
                            $scope.$applyAsync(function() {
                                $scope.nominal = parseFloat(res.nominal);
                            });
                            $('#nominal').val($scope.nominal).trigger("input");
...
function deleteTransaksi(uuid) {
    Swal.fire({
        ...
        if (result.value) {
            ...
            $.ajax({
                url: '/transaksi-biaya-simulasi/' + uuid,
                method: 'DELETE',
