...
app.controller("myCtrl", function($scope,$http,API) {
    angular.element(document).ready(function () {
        table = $("#viewtabel").DataTable({
            processing: true,
            serverSide: true,
            ajax: '{{ route("rekening_bank.datatable") }}',
            scrollY: "50vh",
            scrollX: !0,
            scrollCollapse: !0,
            columns: [
                { data: 'no_rekening', title: 'No Rekening' },
                { data: 'nama_bank', title: 'Bank' },
        ...
        $('#viewtabel tbody').on('click', '#hapus', function () {
            ...
            swal({
                ...
                if(e.value){
                    ...
                    url = "{{ route('rekening_bank.delete',':uuid') }}";
                    url = url.replace(':uuid', x.uuid);
                    $http.delete(url)
                    .then(function(res){
                        if(res.data.success){
                            swal({
                                title: "Terhapus ",text: "Data Rekeing berhasil dihapus!",type: "success",confirmButtonClass: "btn btn-secondary m-btn m-btn--wide"
                            }).then(function(){
                                $('#m_create').modal('hide');
    ...
    $("#formInput").validate({
        ...
        submitHandler: function(e) {
            ...
            let url = ($scope.edit)
                ? "{{ route('rekening_bank.update', ':uuid') }}"
                : "{{ route('rekening_bank.insert') }}";
            ...
            .then(function(res){
                if(res.data.success){
                    swal({
                        title: "Tersimpan ",text: "Data Rekeing berhasil tersiman!",type: "success",confirmButtonClass: "btn btn-secondary m-btn m-btn--wide"
                    }).then(function(){
                        $('#m_create').modal('hide');
                        table.ajax.reload(null, false);
                    })
